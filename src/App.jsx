...
import './App.css'
import PhotoOrbs from './PhotoOrbs'
...
function App() {
  ...
  async function handlePhotosSelected(newPhotos) {
    setPhotos(newPhotos)
    setResults([])
    if (newPhotos.length > 0) setPhase('preview')
    else setPhase('idle')
  }
  ...
  async function runDetection() {
    ...
    for (const photo of photos) {
      ...
      if (!detection) {
        scored.push({ name: photo.name, url: photo.url, noFace: true })
        continue
      }
    ...
    scored.sort((a, b) => {
      const at = a.noFace ? -1 : a.scores.total
      const bt = b.noFace ? -1 : b.scores.total
      return bt - at
    })
    if (scored.length > 0 && !scored[0].noFace) {
      ...
      {phase === 'ranked' && (
        <div>
          ...
          <div className="results__grid">
            {results.map((r, i) => (
              <div key={r.name} className={`photo-card${i === 0 && !r.noFace ? ' photo-card--winner' : ''}`}>
                ...
                <div className="card__body">
                  {r.noFace ? (
                    <p className="card__noface">No face detected</p>
                  ) : (
                    <>
